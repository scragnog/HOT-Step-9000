...
import { lireekApi } from '../../services/lyricStudioApi';
...
export const PromptEditor: React.FC<Props> = ({ open, onClose }) => {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setTimeout(() => setToast(null), 2000);
      await loadPrompts();
      // Re-select to get the default content
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-zinc-900 rounded-2xl border border-white/10 shadow-2xl w-[900px] max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          ...
          <div className="flex items-center gap-2">
            {toast && (
              <span className="text-xs text-green-400 animate-in fade-in">{toast}</span>
            )}
        ...
        <div className="flex flex-1 min-h-0">
          {/* Sidebar — prompt list */}
          <div className="w-56 flex-shrink-0 border-r border-white/5 overflow-y-auto scrollbar-hide py-2">
            {loading ? (
            ...
            {selected ? (
              <>
                ...
                <textarea
                  ...
                  className="flex-1 p-4 bg-black/40 text-sm text-zinc-200 font-mono leading-relaxed resize-none focus:outline-none scrollbar-hide"
                  spellCheck={false}
                  style={{ minHeight: 0 }}
                />
              </>
