...
const DEFAULT_STATE: WindowState = {
  x: -1, // -1 = auto-position on first open
  y: -1,
  width: 340,
  height: 420,
  open: false,
...
export const FloatingPlaylist: React.FC<FloatingPlaylistProps> = ({ onPlaySong, currentSongId }) => {
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      const nx = Math.max(0, Math.min(window.innerWidth - 100, dragStart.current.wx + dx));
      const ny = Math.max(0, Math.min(window.innerHeight - 40, dragStart.current.wy + dy));
      updateWindow({ x: nx, y: ny });
    };
  ...
  const handlePlayMain = useCallback((item: PlaylistItem) => {
    ...
    if (clickedSong) {
      onPlaySong(clickedSong, songList);
    }
  ...
  if (!windowState.open) {
    return (
      <button
        onClick={() => updateWindow({ open: true, minimized: false })}
  ...
  if (windowState.minimized) {
    return (
      <div
        ...
        style={{ left: windowState.x, top: windowState.y, width: windowState.width }}
      >
        <div
          className="flex items-center justify-between px-3 py-1.5 bg-gradient-to-r from-zinc-900 to-zinc-800 rounded-lg border border-white/10 shadow-xl cursor-move"
        ...
        >
          ...
          <div className="flex items-center gap-0.5">
            <button
              onClick={() => updateWindow({ minimized: false })}
            ...
            <button
              onClick={() => updateWindow({ open: false })}
  ...
  return (
    ...
    >
      {/* ── Title bar ────────────────────────────────────────── */}
      <div
        className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-zinc-900 via-zinc-850 to-zinc-900 rounded-t-xl border border-b-0 border-white/10 cursor-move flex-shrink-0"
        onMouseDown={handleDragStart}
      >
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            onClick={() => updateWindow({ minimized: true })}
            className="p-1 text-zinc-500 hover:text-white transition-colors"
            title="Minimize"
          >
          ...
          <div className="divide-y divide-white/5">
            {playlist.items.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`group flex items-center gap-2 px-2.5 py-1.5 hover:bg-white/[0.04] transition-colors ${
                    isItemPlaying ? 'bg-pink-500/10' : ''
                  }`}
                >
                  ...
                  {/* Move / Remove buttons — visible on hover */}
                  <div className="flex items-center gap-0 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <button
                      onClick={() => playlist.move(item.id, 'up')}
                      disabled={idx === 0}
                      className="p-0.5 text-zinc-600 hover:text-white transition-colors disabled:opacity-20"
                      title="Move up"
                    >
                      <ChevronUp className="w-3 h-3" />
                    </button>
                    <button
                      onClick={() => playlist.move(item.id, 'down')}
                    ...
                    <button
                      onClick={() => playlist.remove(item.id)}
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-gradient-to-r from-zinc-900 to-zinc-800 rounded-b-xl border border-t-0 border-white/10 flex-shrink-0">
        {playlist.items.length > 0 ? (
          <button
            onClick={() => {
              if (playlist.items.length > 0) handlePlayMain(playlist.items[0]);
            }}
