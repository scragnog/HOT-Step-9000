...
"""AceStep model configuration — canonical source for all model variants."""
...
class AceStepConfig(PretrainedConfig):
    ...
    Args:
        ...
        lyric_alignment_layers_config (`dict`, *optional*):
            Model-aware lyric alignment attention layers config. Maps layer indices
            (as string keys in JSON) to lists of head indices. When ``None``, the
            handler falls back to the built-in 2B default. XL (4B) models store
            their own config here so that lyric alignment uses the correct layers.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Model-aware lyric alignment config (XL models have different layers)
